...
void CRC16(unsigned char *puchMsg, unsigned short usDataLen, unsigned char *out)
{
...
{
    if (QTcpSocket::UnconnectedState != m_pTemperatureControl->state()) {
        m_pTemperatureControl->disconnectFromHost();
    }
...
{
    unsigned char data[] = {0xFE, 0x04, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00};
    data[3] = nAddr - 1;
    CRC16(data, sizeof(data) - 2, &data[6]);
    m_pTemperatureControl->write(reinterpret_cast<char *>(&data[0]), sizeof(data));
    if (!m_pTemperatureControl->waitForReadyRead()) {
        return 0;
    }
